...
app.post("/wfms", function (req, res, next) {
  ...
  var jobsMatchQuery = 'SELECT Jobs.JobsName, Jobs.JobDescription FROM Skills2Jobs  Join Jobs on Jobs.id = Skills2Jobs.JobID  Join Skills on Skills2Jobs.SkillID = Skills.id  where Skills.SkillsName like  ? ';
  var jobsQuery = "SELECT * FROM Jobs GROUP BY id"
  var industryQuery = "SELECT * FROM Industries GROUP BY id"
